...
@Component({
  ...
  imports: [
    ...
    RouterLink,
    TagModule,
    UserValidatedActivitiesComponent,
    PageHeaderComponent,
  ],
...
export class UserDetailComponent {
  ...
  protected readonly breadcrumbItems = computed<MenuItem[]>(() => [
    ...
    { label: `${this.user()?.firstName} ${this.user()?.lastName}`.trim() },
  ]);
  ...
  protected readonly user = toSignal(
    this.refreshData$.pipe(switchMap(() => this.userRepository.getUserById(this.userId()))),
    { initialValue: null },
  );
